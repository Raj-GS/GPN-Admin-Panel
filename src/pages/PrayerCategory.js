...
const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:2000/api/admin/';
  ...
  return (
    <Box p={2}>
      ...
      <Box mb={10}>
        <Typography variant="h5" fontWeight={700}>Prayer Category Management</Typography>
...
<Button startIcon={<AddIcon/>} variant="contained" onClick={() => setAddCategory(true)} sx={{background: '#177373'}}>Add Category</Button>
</Box>
</Box>
</Paper>
              ...
              <TableRow key={cat.id}>
                ...
                >
                  {cat.name?.[0]?.toUpperCase() || "?"}
                    ...
                    {/* <Avatar src={cat.iconUrl} /> */}
                    {cat.name}
                  </Box>
                </TableCell>
                <TableCell>{cat._count.pray_requests}</TableCell>
                {user.role === 1 && (
                  <TableCell>{cat.origanisation ? cat.origanisation.org_name : 'Default Category'}</TableCell>
                )}
                <TableCell>

                  <Chip
                    label={cat.deleted_at ? 'Deleted' : cat.status}
                    color={cat.deleted_at ? 'error' : cat.status === 'active' ? 'success' : cat.status === 'inactive' ? 'error' : 'default'}
                    size="small"
                  />
                </TableCell>
      ...
      {addCategory && (
        <Dialog open={!!addCategory} onClose={() => setAddCategory(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
              <TextField label="Category Name" value={addCategory.name} onChange={(e) => setAddCategory({ ...addCategory, name: e.target.value })} />
            </Box>
            <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 2 }}>
...
export default PrayerCategoryList;
